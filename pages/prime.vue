<template>
  <div class="container">
        ...
        <div class="tableD">
          ...
          <div class="header"> 
            ...
            <div style="display: flex; align-items:center; width: 100%; height: 40px; border-bottom-color: rgb(226, 226, 226); border-bottom-width: 1px; border-bottom-style: solid; padding: 18px;"  >
              <div style="display: flex; align-items: center;"> 
              <!-- <div> <Icon name="teenyicons:database-solid" style="font-size: 14px; margin-right: 10px;"/> </div> -->
                  <div style="font-size:16px; font-weight:350; margin-right: 10px;" > Tabla </div>
                    /
                </div>
                <div @click="toggleTables" > 
                  <span style="font-size: 16px; font-weight: 350; margin-left: 10px;"> {{ this.selectedTableName }}  </span> <Icon name="quill:expand" style="font-size: 15px; margin-top:-3px; margin-left:1px; margin-right:10px;" /> 
                </div>
            </div>
          ...
          <div> 
            ...
            <div style="font-size:12px; margin-bottom:10px;" > Selecciona Tabla
              <div style="margin-top: 4px;" > 
                
                <Listbox v-model="selectedTableName" :options="tables" filter optionLabel="" style="font-size:13px; max-height: 350px; width: 300px; overflow-y: scroll;" />
            ...
            <div style="height: 40px;">
              ...
              <Button type="button" label="Recargar" iconPos="left" size="small" class="p-button-text" @click="reloadData">
                <span v-if="loading">
                    <Icon name="svg-spinners:bars-rotate-fade" style="font-size: 15px; margin-right: 10px;"/>
                </span>
                <span v-else>
                    <Icon name="uil:spinner-alt" style="font-size: 15px; margin-right: 10px;"/>
                </span> Recargar
            </Button>
    
              <Button icon="pi pi-table" type="button" label="Columnas" iconPos="left" size="small"  class="p-button-text" @click="toggleColumn" />  
              ...
              <Button icon="pi pi-filter" type="button" label="Filtrar" iconPos="left" size="small"  class="p-button-text" @click="toggleFilter" />
              <OverlayPanel ref="filter">
                ...
                <Button icon="pi pi-plus" type="button" label="Agregar" iconPos="left" size="small"  class="p-button-text" @click="toggleAdd" />
              <OverlayPanel ref="add">
          
                <div @click="toggleNewItemSidebar"  class="itemAgregar"> 
                  <Icon  name="bx:columns" style="font-size: 18px; margin-right: 10px;"/> 
                   Insertar 
                   <span style="font-weight:600;">{{this.selectedTableName}}</span>
                </div>

                <div style="font-size: 13px;"> <Icon name="clarity:view-columns-line" style="font-size: 18px; margin-right: 10px;"/> Agregar Columna</div>
      ...
      <DataTable 
        ...
        :sortable="false"
        :frozen="true"
        style="min-width: 70px; z-index:100; border-right-color:rgb(233, 233, 233); border-right-style: solid; border-right-width: 1px;">
        
        <template v-slot:body="slotProps">
          {{ slotProps.data.index }} 
          <Icon name="system-uicons:expand" class="row-hover-icon" :style="{ opacity: slotProps.rowIndex === hoveredRowIndex ? 1 : 0 }" />
        </template>
      ...
      <Column v-for="col in columns" 
        ...
        <template v-else>
          ...
          <Icon name="system-uicons:expand" 
                class="row-hover-icon" 
                :style="{ opacity: slotProps.rowIndex === hoveredRowIndex ? 1 : 0 }" />
 ...
 <Sidebar 
   v-model:visible="sidebarVisible"  position="right" :style="{ width: '50%' }">

   <!-- Encabezado personalizado con estilos -->
   
   <template #header>
      <h3 style="font-weight:300; font-size: 15px; padding: 0px;">Editar fila desde  <Tag :value="this.selectedTableName" class="custom-tag"> </Tag>
      </h3>
    </template>
    ...
    <form @submit.prevent="saveChanges" style="font-weight:300; font-size: 14px; padding: 10px; padding-top:0px;" >
     
      <div v-for="key in getFilteredRowDataKeys()" :key="key"  >
       
      <div class="formItem">
        
        <div style="min-width: 250px; display: flex; flex-direction: row; margin-bottom: 7px; align-items:center; ">
            <div  style="font-size:11px">{{ key }}  </div>
            <div style="font-size: 9px; color: rgb(165, 165, 165); margin-left:7px ">{{ getColumnDetail(key).data_type }} </div>
        </div>

          <div style=" width: 100%;" >
            <Editor v-if="getColumnDetail(key).data_type === 'character varying'" v-model="selectedRowData[key]" editorStyle="height: 220px" />
            <Textarea v-if="['text', 'json'].includes(getColumnDetail(key).data_type)" class="inputInside"
            :id="key" v-model="selectedRowData[key]" autoResize rows="5" cols="30" />
          <Checkbox class="inputInside" v-else-if="getColumnDetail(key).data_type === 'boolean'" :id="key" v-model="selectedRowData[key]" binary />
            <Calendar class="inputInside"  v-else-if="getColumnDetail(key).data_type === 'timestamp with time zone' || getColumnDetail(key).data_type === 'timestamp without time zone'" :id="key" v-model="selectedRowData[key]" showTime hourFormat="12" showIcon iconDisplay="input" :disabled="key === 'created' || key === 'update' || key === 'created_at' || key === 'id' " />
            <InputText style="max-height:300px;" class="inputInside" v-else-if="getColumnDetail(key).data_type === 'uuid' " :id="key" v-model="selectedRowData[key]" disabled placeholder="Deshabilitado" />
            <InputNumber class="inputInside" v-else-if="getColumnDetail(key).data_type === 'bigint'" :id="key" v-model="selectedRowData[key]" :disabled="key === 'id'" />
            <InputNumber class="inputInside" v-else-if="getColumnDetail(key).data_type === 'numeric'" :id="key" v-model="selectedRowData[key]" :useGrouping="false" :minFractionDigits="2" :maxFractionDigits="4" />
            <input class="inputInside" v-else :type="getInputType(getColumnDetail(key).data_type)" :id="key" v-model="selectedRowData[key]" />
...
<Button style="margin-left: 20px;" label="Eliminar" class="p-button-text p-button-danger" @click="deleteRecord" ></Button>
...
<Sidebar v-model:visible="newItemSidebarVisible" position="right" :style="{ width: '50%' }">

  <template #header>
      <h3 style="font-weight:300; font-size: 15px; padding: 0px;">Insertar fila desde  <Tag :value="this.selectedTableName" class="custom-tag"> </Tag>
      </h3>
    </template>


    <form @submit.prevent="addNewRow" style="font-weight:300; font-size: 14px; padding: 10px; padding-top:0px;">
      <div v-for="key in getCreationRowDataKeys()" :key="key">
        <div class="formItem">
          ...
          <div style="width: 100%;">
            ...
            <Editor v-if="getColumnDetail(key).data_type === 'character varying'" v-model="newRowData[key]" editorStyle="height: 220px" />
            <Textarea v-if="['text', 'json'].includes(getColumnDetail(key).data_type)" class="inputInside" :id="key" v-model="newRowData[key]" autoResize rows="5" cols="30" />
            <Checkbox class="inputInside" v-else-if="getColumnDetail(key).data_type === 'boolean'" :id="key" v-model="newRowData[key]" binary />
            <Calendar 
          class="inputInside" 
          v-else-if="['timestamp with time zone', 'timestamp without time zone'].includes(getColumnDetail(key).data_type)" 
          :id="key" 
          ...
          :placeholder="key === 'created_at' ? new Date().toLocaleString() : ''" 
        />
          <InputText style="max-height:300px;" class="inputInside" v-else-if="getColumnDetail(key).data_type === 'uuid'" :id="key" v-model="newRowData[key]" disabled placeholder="Deshabilitado" />
            <InputNumber 
          class="inputInside" 
        ...
        />  
        <InputNumber class="inputInside" v-else-if="getColumnDetail(key).data_type === 'numeric'" :id="key" v-model="newRowData[key]" :useGrouping="false" :minFractionDigits="2" :maxFractionDigits="4" />
            <input class="inputInside" v-else :type="getInputType(getColumnDetail(key).data_type)" :id="key" v-model="newRowData[key]" />
          </div>
        </div>
      </div>
 ...
 <Toast ref="toast" />
  ...
  try {
    ...
    this.reloadData();
    this.$toast.add({ severity: 'success', summary: 'Eliminado', detail: 'Registro eliminado exitosamente', life: 3000 });
  ...
  try {
    ...
    const { error } = await supabase.from(this.selectedTableName).insert([newRow]);

    if (error) throw error;
    this.newItemSidebarVisible = false;
    this.$toast.add({severity:'success', summary: 'Creado', detail: 'Registro creado exitosamente', life: 3000});
    ...
    this.$toast.add({severity:'success', summary: 'Guardado', detail: 'Guardado exitosamente', life: 3000});
    ...
    getFilteredRowDataKeys() {
        return Object.keys(this.selectedRowData)
          .filter(key => {
            ...
            const columnDetail = this.columnDetails.find(detail => detail.column_name === key);
    ...
    return this.columnDetails.find(detail => detail.column_name === columnName) || {};
  },

  getInputType(columnName) {
    const column = this.columnDetails.find(col => col.column_name === columnName);
    if (column) {
      ...
      async fetchTables() {
         try {
           ...
           const { data, error } = await supabase
             .rpc('get_tables', { target_schema: 'public' });
       ...
       async fetchColumnDetails() {
        try {
          ...
          console.error('Error al obtener detalles de las columnas:', error.message);
        }
      },
   ...
   subscribeToRealtimeUpdates() {
     if (this.selectedTableName) {
       console.log("Subscribiendo a cambios en tiempo real para la tabla:", this.selectedTableName);
    ...
    handleRealtimeUpdate(payload) {
     ...
     switch (payload.eventType) {
       ...
       const indexToUpdate = this.products.findIndex(item => item.id === record.id);
       if (indexToUpdate !== -1) {
       ...
       case 'DELETE':
         ...
         this.products = this.products.filter(item => item.id !== payload.old.id);
         break;
